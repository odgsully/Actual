...
import { WeeklyPreview } from '@/components/tiles/printoffs/previews/WeeklyPreview';

export default function WeeklyPrintoffPage() {
  const [isPrinting, setIsPrinting] = useState(false);

  const handlePrint = async () => {
    setIsPrinting(true);
    try {
      const response = await fetch('/api/reports/weekly/print', {
        method: 'POST',
      });
  ...
  const handlePreviewFull = () => {
    window.open('/reports/weekly/preview', '_blank');
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Schedule: Sunday 5 AM</p>
          </div>
