...
import { MonthlyPreview } from '@/components/tiles/printoffs/previews/MonthlyPreview';

export default function MonthlyPrintoffPage() {
  const [isPrinting, setIsPrinting] = useState(false);

  const handlePrint = async () => {
    setIsPrinting(true);
    try {
      const response = await fetch('/api/reports/monthly/print', {
        method: 'POST',
      });
  ...
  const handlePreviewFull = () => {
    window.open('/reports/monthly/preview', '_blank');
  };
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Monthly Report</h1>
            <p className="text-muted-foreground text-sm">Schedule: 1st of month, 5 AM</p>
          </div>
