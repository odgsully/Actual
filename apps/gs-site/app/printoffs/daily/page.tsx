...
import { DailyPreview } from '@/components/tiles/printoffs/previews/DailyPreview';

export default function DailyPrintoffPage() {
  const [isPrinting, setIsPrinting] = useState(false);

  const handlePrint = async () => {
    setIsPrinting(true);
    try {
      const response = await fetch('/api/reports/daily/print', {
        method: 'POST',
      });
  ...
  const handlePreviewFull = () => {
    window.open('/reports/daily/preview', '_blank');
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Schedule: 5 AM daily</p>
          </div>
