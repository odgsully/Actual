...
// Demo email can be configured via environment variable
const DEMO_EMAIL = process.env.NEXT_PUBLIC_DEMO_USER_EMAIL || 'support@wabbit-rank.ai'
...
// Use flexible type to accept any Supabase client configuration
type AuthProviderProps = {
  children: ReactNode
  createClient: () => SupabaseClient<any, any, any>
...
export function AuthProvider({
  ...
  const signUp = async (email: string, password: string, metadata?: any) => {
    try {
      ...
      if (data.user) {
        const response = await fetch(signupApiRoute, {
          method: 'POST',
          headers: {
